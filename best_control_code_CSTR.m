...
for n=1:N
    ...
    str=sprintf('Tracking error: %g', trcke(n));
    disp(str)
...
linewidth=2.5;
markersize=2.5;
time=Ts*(0:size(yref_yaw, 2)-1)';
figure,
plot(time, y_yaw, 'r', 'LineWidth', linewidth*0.25)
hold on figure,
plot(time, yref_yaw, 'k:', 'MarkerSize', markersize, 'LineWidth', linewidth*1)
xlabel('t(s)')
ylabel('yref(t), y(t)')
str=sprintf('P: %g, I:, %g, D: %g, SNR=%g dB', Kp, Ki, Kd, SNR);
title(str)
legend('actual trajectory', 'reference trajectory')
box off,

figure,
plot(time, u_yaw, 'k', 'LineWidth', linewidth*0.5)
xlabel('t(s)')
ylabel('u(t)')
title('u(t): produced control input by PID')
box off,